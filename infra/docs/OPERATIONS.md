...
Terminate TLS and authenticate users at the corporate gateway. The gateway must remove client-supplied identity headers and set the trusted-proxy secret and scoped user headers documented in the API repository's README. Store database, proxy, and approved AI credentials in the platform secret manager; never bake them into images.
