...
function initialRoute(preferences) {
  ...
  const section = stakeholder ? "Stakeholder Map" : opportunity ? "Account Data" : meeting && requestedSection !== "Pod View" ? "Account Data" : requestedSection;
  return {
...
function EnterpriseFunctions({ groups, onSelect, matches, selectedId }) {
  // Enterprise cards contain the same hierarchy as business units, but used
  // fixed, equal columns. Let each card reserve enough room for every branch
  // so a report (for example, someone in the HR team) cannot sit outside it.
  const groupWidths = groups.map((group) => {
    const slots = group.roots.reduce((total, root) => total + branchSlots(root), 0);
    return Math.max(280, 68 + slots * 172);
  });
  return (
    <section className="enterprise-functions">
      <div className="enterprise-title"><span>Enterprise Functions</span></div>
      <div className="enterprise-grid" style={{ gridTemplateColumns: groupWidths.map((width) => `${width}px`).join(" ") }}>
        {groups.map((group) => {
...
function MapCanvas({ data, query, filters, onSelect, editMode, panelMode, selectedId, focusRequest }) {
  ...
  const reset = () => { setZoom(defaultZoom); setOffset(centeredOffset(14, defaultZoom)); };
  useEffect(() => {
    setZoom(defaultZoom);
    setOffset({ x: 18, y: 14 });
  }, [defaultZoom]);
  ...
  // A lane needs room for the technology column, the business-tree padding,
  // and the gaps between every branch.  Sizing only from the branch count
  // made the last name in a wide team extend beyond its unit and division.
  const laneWidth = Math.max(552, 232 + widestBusinessRow * 190);
  useEffect(() => {
...
function App() {
  ...
  useEffect(() => {
    if (!podOptions.length) { if (pod !== "All") setPod("All"); return; }
