...
const validPods = new Set(["All", "ISG", "Wealth Management", "MSIM"]);
...
export function loadWorkspacePreferences(storage) {
  ...
  try {
    ...
    return {
      defaultPod: validPods.has(value.defaultPod) ? value.defaultPod : DEFAULT_WORKSPACE_PREFERENCES.defaultPod,
...
export function resolveInitialPod(search, preferences = DEFAULT_WORKSPACE_PREFERENCES, section = "Executive View") {
  ...
  return validPods.has(requested) ? requested : preferences.defaultPod;
}
