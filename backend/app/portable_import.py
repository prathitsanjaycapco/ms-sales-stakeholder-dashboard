"""Validated account dataset schema shared by Excel and portable imports."""
...
class PortableImportBatch(ImportRow):
    ...
    def references_are_valid(self):
        ...
        for label, values in (
            ("division names within a pod", [f"{item.pod}/{item.name}" for item in self.divisions]),
            ("business unit names within a division", [f"{item.division_id}/{item.name}" for item in self.business_units]),
            ("employee source IDs", [item.source_record_id for item in self.employees if item.source_record_id]),
            ("stakeholder source IDs", [item.source_record_id for item in self.stakeholders if item.source_record_id]),
        ):
            duplicates = _duplicates(values)
            if duplicates:
...
def insert_import_batch(connection, batch: PortableImportBatch) -> dict[str, int]:
